...
check_and_build_images() {
    ...
    if ! docker images | grep -q "game-mods-mcp-opencode"; then
        echo "📦 OpenCode MCP image not found, building..."
        images_missing=true
    fi

    # Check for mcp-crush image
    if ! docker images | grep -q "game-mods-mcp-crush"; then
        echo "📦 Crush MCP image not found, building..."
        images_missing=true
    fi

    # Check for openrouter-agents image
    if ! docker images | grep -q "game-mods-openrouter-agents"; then
    ...
    if [ "$images_missing" = true ]; then
        ...
        docker build -f docker/openrouter-agents.Dockerfile \
            --build-arg OPENCODE_IMAGE=game-mods-mcp-opencode:latest \
            --build-arg CRUSH_IMAGE=game-mods-mcp-crush:latest \
            -t game-mods-openrouter-agents:latest .
